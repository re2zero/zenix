...
impl WgpuContext {
    ...
    async fn try_adapter_with_surface(
        ...
        // Test vertex shader storage buffer support.
        // On some GL implementations with old GPUs (e.g. AMD CAICOS),
        // vertex shader storage blocks are not supported even though
        // GL_ARB_shader_storage_buffer_object is advertised. This test
        // catches that by trying to create a minimal render pipeline
        // that uses a vertex storage buffer.
        {
            let internal_scope = device.push_error_scope(wgpu::ErrorFilter::Internal);

            let test_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("vertex_storage_cap_test"),
                source: wgpu::ShaderSource::Wgsl(std::borrow::Cow::Borrowed(
                    "@group(0) @binding(0) var<storage,read> d:array<vec4<f32>>;\
                     @vertex fn v()->@builtin(position)vec4<f32>{return d[0];}\
                     @fragment fn f()->@location(0)vec4<f32>{return vec4(1.);}",
                )),
            });

            let test_bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("vertex_storage_cap_test_bgl"),
            ...
            let test_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("vertex_storage_cap_test_layout"),
                bind_group_layouts: &[Some(&test_bgl)],
                immediate_size: 0,
            });

            let _ = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("vertex_storage_cap_test_pipeline"),
            ...
            if let Some(error) = internal_scope.pop().await {
                anyhow::bail!(
                    "vertex shader storage buffers not supported on this adapter: {error}"
                );
            }
        }
